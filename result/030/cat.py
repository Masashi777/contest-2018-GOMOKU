...
def LongestPoint(field, position):
    dists = [CountStonesOnLine(field, position, (1, 1)),
     CountStonesOnLine(field, position, (1, 0)),
     CountStonesOnLine(field, position, (1, -1)),
     CountStonesOnLine(field, position, (0, 1)),
     ]
    return max(dists)
...
def Think(field):
    ...
    for i in range(N):
        for j in range(N):
            ...
            field[i][j] = '.'
            if LongestPoint(field, position) > LongestPoint(field, best_position):
