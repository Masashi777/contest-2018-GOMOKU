...
def Think(field):
    ...
    for i in range(N):
        for j in range(N):
            ...
            field[i][j] = 'O'
            if CanHaveNStones(field, position, 5, 'O'):
                DebugPrint('I have a winning choice at (%d, %d)' % (i, j))
                return position

            field[i][j] = 'X'
            # look for 'X'
            if CanHaveNStones(field, position, 5, 'X'):
                DebugPrint('block 4 at (%d, %d)' % (i, j))
                return position

            field[i][j] = 'O'
            # four
            if CanHaveNStones(field, position, 4, 'O'):
                DebugPrint('4 at (%d, %d)' % (i, j))
                return position
