...
def Think(field):
    ...
    for i in range(N):
        for j in range(N):
            ...
            field[i][j] = 'X'
            if CanHaveFourStones(field, position, 'X'):
                return position
...
def CanHaveFourStones(field, position, mark):
    if CountStonesOnLine(field, position, (1, 1), mark) >= 4:
        if field[position[0] + 1][position[1] + 1] == mark:
            return field[position[0] + 4][position[1] + 4] == '.'
        if field[position[0] - 1][position[1] - 1] == mark:
            return field[position[0] - 4][position[1] - 4] == '.'
    if CountStonesOnLine(field, position, (1, 0), mark) >= 4:
        if field[position[0] + 1][position[1]] == mark:
            return field[position[0] + 4][position[1]] == '.'
        if field[position[0] - 1][position[1]] == mark:
            return field[position[0] - 4][position[1]] == '.'
    if CountStonesOnLine(field, position, (1, -1), mark) >= 4:
        if field[position[0] + 1][position[1] - 1] == mark:
        ...
        if field[position[0] - 1][position[1] + 1] == mark:
            return field[position[0] - 4][position[1] + 4] == '.'
    if CountStonesOnLine(field, position, (0, 1), mark) >= 4:
        if field[position[0]][position[1] + 1] == mark:
            return field[position[0]][position[1] + 4] == '.'
        if field[position[0]][position[1] - 1] == mark:
            return field[position[0]][position[1] - 4] == '.'
