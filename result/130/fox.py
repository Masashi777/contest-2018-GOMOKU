...
def scoring(field):
    ...
    # 3 no side (300)
    # 2 no side (200)
    num4 = CanHaveNStones(field, 4)
    num3 = CanHaveNStones(field, 3)
    num2 = CanHaveNStones(field, 2)
    return num4*99999 + num3*300 + num2*200
        ...
        for j in range(N):
            if field[i][j] == 'O':
                ...
                for d in [(1, 1), (1, 0), (1, -1), (0, 1)]:
                    ...
                    if (CountStonesOnLine(field, position, d, 'O') == n and 
                        field[i-d[0]][j-d[1]] == '.' and field[i+n*d[0]][j+n*d[1]] =='.'):
                        count += 1
