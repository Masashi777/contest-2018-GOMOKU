...
def Think(field):
    ...
    for i in range(N):
        for j in range(N):
            ...
            field[i][j] = 'X'
            if OppHaveFourStones(field, position):
                DebugPrint('I have a winning choice at (%d, %d)' % (i, j))
                return position
            field[i][j] = 'O'
            count = 0
            if CanHaveFiveStones(field, position, count):
                    DebugPrint('I have a winning choice at (%d, %d)' %(i, j))
                    if count > canMaxStone:
...
def OppHaveFourStones(field, position):
    return (CountOppStonesOnLine(field, position, (1, 1)) >= 4 or
            CountOppStonesOnLine(field, position, (1, 0)) >= 4 or
            CountOppStonesOnLine(field, position, (1, -1)) >= 4 or
            CountOppStonesOnLine(field, position, (0, 1)) >= 4)
...
def CanHaveFiveStones(field, position, count):
    return (CountStonesOnLine(field, position, (1, 1), count) >= 5 or
            CountStonesOnLine(field, position, (1, 0), count) >= 5 or
            CountStonesOnLine(field, position, (1, -1), count) >= 5 or
            CountStonesOnLine(field, position, (0, 1), count) >= 5)


# Returns the number of stones you can put around |position| in the direction specified by |diff|.
def CountStonesOnLine(field, position, diff, num):
    count = 0
