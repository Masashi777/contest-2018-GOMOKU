...
def Think(field):
    ...
    for i in range(N):
        for j in range(N):
            ...
            field[i][j] = 'O'
            mark = 'O'
            if CanHaveFiveStones(field, position, mark):
            ...
            field[i][j] = 'X'
            mark = 'X'
            if CanHaveFourStones(field, position, mark):
                return position
...
def CanHaveFourStones(field, position, mark):
    return (CountStonesOnLine(field, position, (1, 1), mark) >= 4 or
            CountStonesOnLine(field, position, (1, 0), mark) >= 4 or
            CountStonesOnLine(field, position, (1, -1), mark) >= 4 or
            CountStonesOnLine(field, position, (0, 1), mark) >= 4)
